...
class Model_realty extends CI_Model{
	...
	function publishItem(){
		...
		$sqlInsertRealty = "INSERT INTO `realty`(`REALTYID`, `ADDRESSID`, `USERNAME`, `KIND`, `TYPE`, `TITLE`, `PRICE`, `SIZE`, `DESCRIPTION`, `TAG`, `PUBLISHDATE`, `STATUS`, `CONTACTNAME`, `CONTACTTEL`, `CONTACTEMAIL`, `CONTACTADDRESS`) VALUES (NULL, '$addressID', 'HuongHk', '$kind', '$type', '$title', '$price', '$size', '$description', 'tag', '2015-01-04', 'active', '$contactName', '$contactTel', '$contactEmail', '$contactAddress')";
	...
	public function uploadFile(){
		if(isset($_FILES['files'])){
			...
			foreach($_FILES['files']['tmp_name'] as $key => $tmp_name ){
				...
				$desired_dir="C:/xampp/htdocs/batdongsan.com/resource/images/";
	...
	public function showListItem($kind, $type, $districtID, $provinceID, $size, $price, $limit, $offset){
	...
	public function getTotalItem($kind, $type, $districtID, $provinceID, $size, $price)
	{
		$sql = "SELECT `DISTRICTNAME` FROM `district` WHERE `DISTRICTID` = $districtID";
		$query = $this->db->query($sql);
		$result = $query->result();
